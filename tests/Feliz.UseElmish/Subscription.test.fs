...
module Subscriber =
    ...
    type Model = { Count: int; SubscriberTicks: int }
    ...
    let update msg (model: Model) =
        ...
        | SubscriberTick ->
            {
                model with
                    SubscriberTicks = model.SubscriberTicks + 1
            },
            Cmd.none

    let subscribeToTimer dispatch =
        let subscriptionId = setInterval (fun _ -> dispatch SubscriberTick) 100

        { new System.IDisposable with
            member _.Dispose() = clearInterval subscriptionId
        }

    let subscribe _model = [ [ "timer" ], subscribeToTimer ]

    [<ReactComponent>]
    let Render (initialValue: int) =
        let model, dispatch =
            React.useElmish ((fun () -> init initialValue), update, subscribe, [||])

        Html.div [
            Html.h1 [ prop.testId "count"; prop.text model.Count ]
...
describe "UseElmish with subscriptions"
<| fun () ->
    testPromise "Elmish subscriptions work correctly"
    <| fun () -> promise {
        ...
        do!
            RTL.waitFor (fun () -> expect(render.getByTestId ("count")).toHaveTextContent "1" //"Count should have been incremented"
            )
    }
