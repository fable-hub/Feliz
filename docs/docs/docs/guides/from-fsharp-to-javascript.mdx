...
The transpiled code in its current state looks rather obscure and is not very ergonomic to use from JavaScript. Check it out if you are interested, but I will highlight the relevant parts below.
...
- `classNames` is of type `FSharpList<string>`, which is not very ergonomic to use from JavaScript. A simple `string[]` would be easier to understand.
...
Discriminated unions are not very ergonomic to use from JavaScript. A simple string enum is much easier to use.
...
And that's it! We have transformed our F# code to be much more JavaScript friendly! 🎉
