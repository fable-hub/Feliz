...
### Using `prop.spread` with object props

When your props come from a JavaScript object (for example from interop code), use `prop.spread` to convert that object into `IReactProperty[]`, then `yield!` the result.
...
let dynamicProps =
    createObj [
        ...
        "onClick" ==> (fun _ -> Browser.Dom.console.log("clicked"))
    ]
...
`prop.spread` is dynamic and does not provide the same compile-time guarantees as explicit Feliz props. Prefer explicit typed props whenever possible, and use spreading when interop or dynamic scenarios require it.
